...
class TransactionViewItemFactory(
    ...
    private fun convertToViewItem(transactionItem: TransactionItem): TransactionViewItem {
        ...
        return when (record) {
            ...
            is StellarCreateAccountTransactionRecord -> {
                createViewItemFromStellarCreateAccountTransactionRecord(
                    record = record,
                    progress = progress,
                    icon = icon,
                )
            ...
            is StellarPaymentTransactionRecord -> {
                createViewItemFromStellarPaymentTransactionRecord(
                    record = record,
                    progress = progress,
                    icon = icon,
                )
    ...
    private fun createViewItemFromStellarTransactionRecord(
        ...
        return TransactionViewItem(
            ...
            secondaryValue = null,
            date = Date(record.timestamp * 1000),
            icon = icon ?: TransactionViewItem.Icon.Platform(BlockchainType.Stellar)
        )
    }

    private fun createViewItemFromStellarCreateAccountTransactionRecord(
        record: StellarCreateAccountTransactionRecord,
        progress: Float?,
        icon: TransactionViewItem.Icon?,
    ): TransactionViewItem {
        return TransactionViewItem(
    ...
    private fun createViewItemFromStellarPaymentTransactionRecord(
        record: StellarPaymentTransactionRecord,
        progress: Float?,
        icon: TransactionViewItem.Icon?,
    ): TransactionViewItem {
        val isSend = record.accountId == record.operation.from
        val title =
            if (isSend) Translator.getString(R.string.Transactions_Send) else Translator.getString(R.string.Transactions_Receive)
        val subtitle =
            if (isSend) {
                Translator.getString(R.string.Transactions_To, record.operation.from.shorten())
            } else {
                Translator.getString(R.string.Transactions_From, record.operation.from.shorten())
            }
        ...
        val primaryValue =
            if (isSend) {
                ...
                getColoredValue(record.value, ColorName.Remus)
            }
        return TransactionViewItem(
            uid = record.uid,
            progress = progress,
