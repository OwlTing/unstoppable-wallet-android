...
class RestoreBlockchainsService(
    ...
    fun restore() {
        ...
        val wallets = enabledTokens.map { Wallet(it, account) }.toMutableList()

        val enabledBlockchain = items.filter { it.enabled }.map { it.blockchain.type }
        wallets.addAll(SupportedTokenHelper.createUSDCWallets(account, enabledBlockchain))
